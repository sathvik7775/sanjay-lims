...
const isPublicPage = window.location.pathname.startsWith("/public");
  ...
  // 🔄 Sync branchToken back to localStorage whenever it changes
  useEffect(() => {
    if (branchToken) {
  ...
  useEffect(() => {
    ...
    if (savedToken) {
      setBranchToken(savedToken);
    }

    if (savedInfo) {
      const parsed = JSON.parse(savedInfo);
      setBranchId(parsed._id);
  ...
  useEffect(() => {
    ...
    if (token && token !== "null" && token !== "undefined") {
      ...
      delete axios.defaults.headers.common["Authorization"]; // prevent invalid token
    }
  }, [adminToken, branchToken]);
